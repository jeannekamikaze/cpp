...
Exception::Exception(const std::string& what) throw() {
  snprintf(exp_buf, EXP_SIZE, "%s", what.c_str());
}
...
Exception::Exception(const char* file, int line,
  ...
  snprintf(exp_buf, EXP_SIZE, "File: %s, line: %d: %s", file, line,
           what.c_str());
}

Exception::Exception(const char* file, int line,
                     const std::ostringstream& what) throw() {
  snprintf(exp_buf, EXP_SIZE, "File: %s, line: %d: %s", file, line,
           what.str().c_str());
}
...
#ifdef EXCEPTION_BOOST_FORMAT
Exception::Exception(const char* file, int line,
                     const boost::format& format) throw() {
  snprintf(exp_buf, EXP_SIZE, "File: %s, line: %d: %s", file, line,
           format.str().c_str());
}
#endif

const char* Exception::what() const throw() { return exp_buf; }
